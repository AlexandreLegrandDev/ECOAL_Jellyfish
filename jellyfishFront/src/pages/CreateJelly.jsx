...
import {
    ...
    PaintBucket,
    TriangleAlert,
    Diameter, Lightbulb, TextWrap, LogOut
} from "lucide-react";
...
const JellyIcon = ({className}) => (
    ...
    >
        <path
            d="M12 2C7.58 2 4 5.58 4 10c0 1.66 1.34 3 3 3s3-1.34 3-3V8c0-1.1.9-2 2-2s2 .9 2 2v2c0 1.66 1.34 3 3 3s3-1.34 3-3c0-4.42-3.58-8-8-8z"/>
...
function CreateJelly() {
    ...
    const handleCreate = async () => {
        if (!collectionId) {
            ...
            // alert("User:" + JSON.stringify(token, null, 2));
            ...
            const res = await fetch(`${API}/jellyfish`, {
                ...
                body: JSON.stringify({imagePreview, size, deep, color, diameter, description}),
            });
    ...
    return (
        <div className="w-full min-h-screen bg-bg-dark text-white p-6 relative pb-10">
            ...
            {/* Image Upload Area */}
            <div
                className="relative w-full aspect-square max-h-[250px] bg-[#292B57]/50 rounded-3xl border border-accent-purple/40 shadow-[0_0_20px_rgba(174,48,208,0.2)] flex items-center justify-center mb-8 cursor-pointer hover:bg-[#292B57]/70 transition-colors overflow-hidden">
                {imagePreview ? (
                    ...
                    <Plus size={80} className="text-white font-bold drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]"
                          strokeWidth={2.5}/>
                )}
                <input
                    type="file"
            ...
            <div className="flex flex-col">
                <InputField icon={<Ruler className="w-5 h-5"/>} label="Size" value={size}
                            onChange={(e) => setSize(e.target.value)}/>
                <InputField icon={<ArrowDownFromLine className="w-5 h-5"/>} label="Deep" value={deep}
                            onChange={(e) => setDeep(e.target.value)}/>
                <InputField icon={<PaintBucket className="w-5 h-5"/>} label="Color" value={color}
                            onChange={(e) => setColor(e.target.value)}/>

                {/* Danger rating */}
                <div
                    className="relative flex items-center bg-[#0B0D28]/80 border border-accent-purple/50 rounded-full px-4 py-3 mb-4 shadow-[0_0_10px_rgba(174,48,208,0.15)]">
                    <div className="mr-3 text-[#A855F7] opacity-80 shrink-0">
                ...
                <InputField icon={<Diameter className="w-5 h-5"/>} label="Diameter" value={diameter}
                            onChange={(e) => setDiameter(e.target.value)}/>

                {/* Light toggle */}
                <div
                    className="relative flex items-center bg-[#0B0D28]/80 border border-accent-purple/50 rounded-full px-4 py-3 mb-4 shadow-[0_0_10px_rgba(174,48,208,0.15)] max-w-max pr-8">
                    <div className="mr-3 text-[#A855F7] opacity-80 shrink-0">
                ...
                <div
                    className="relative flex bg-[#0B0D28]/80 border border-accent-purple/50 rounded-[30px] overflow-hidden px-4 py-3 mb-8 shadow-[0_0_10px_rgba(174,48,208,0.15)] min-h-[120px]">
